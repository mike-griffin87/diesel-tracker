...
export default async function Home({
  ...
  const yearParamRaw = Array.isArray(searchParams?.year)
    ? (searchParams?.year[0] as string)
    : (searchParams?.year as string | undefined);
  ...
  const delIdRaw = searchParams?.delete;
